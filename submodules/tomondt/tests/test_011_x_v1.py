...
class TestMethods_010x(unittest.TestCase):
    ...
    def test_0110_readwrite_new(self):
        ...
        helpers.update_data_path(self.data_path, vmf_path, "test_load_vmf_v1.vmf")

    def test_0111_readwrite_loaded(self):
        name = self.testnum+str(1)
        vmf_path = self.files[0]
    ...
    """ same as test 0112 but without a zero time record on init"""
    def test_0112_readwrite_new(self):
        name = self.testnum+str(2)
        vmf_path = os.path.join(self.path, name + "_v1.vmf")
        vmf = tomondt.load_vmf(vmf_path,version=(1,0,0))
    ...
    """Same as Test 0112 but with attached notes"""
    def test_0113_readwrite_new(self):
        name = self.testnum+str(3)
        vmf_path = os.path.join(self.path, name + "_v1.vmf")
        vmf = tomondt.load_vmf(vmf_path,version=(1,0,0))

        note = "Test Note: This is a test I'm just making sure I can add basic file information: Sample Name Sim_001 Nanocage Phantom collected from previous paper"
        timeunit = "ms"
        spaceunit = "um"

        vmf.notes = note
        vmf.timeunit = timeunit
        vmf.spaceunit = spaceunit
        ...
        vmf = tomondt.load_vmf(vmf_path,version=(1,0,0))
        self.assertEqual(vmf.notes, note)
        self.assertEqual(vmf.timeunit, timeunit)
        self.assertEqual(vmf.spaceunit, spaceunit)
